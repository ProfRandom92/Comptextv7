...
def test_benchmark_suite_covers_strong_and_weak_cases() -> None:
    cases = benchmark_cases()

    assert len(cases) >= 4
    assert any("Best case" in case.expectation for case in cases)
    assert sum("Weak case" in case.expectation for case in cases) >= 2


def test_benchmark_results_are_structured_and_honest_about_expansion() -> None:
